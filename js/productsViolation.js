app.controller(
	...
	function ($scope, $http, $translate, $rootScope, $location) {
		...
		$scope.currentViolationPage = 0;
		$scope.totalViolationPages = 0;
		$scope.pageViolationSize = 9;

		var url = "http://localhost:8080";

		$scope.getproductList = function (currentPage) {
			$http
				.get(url + "/staff/violationsproduct/" + currentPage)
				.then(function (res) {
		...
		$scope.getproductViolationList = function (currentPage) {
			$http
				.get(url + "/staff/alreadyviolationsproduct/" + currentPage)
				.then(function (res) {
					var products = res.data.content; // Lấy danh sách sản phẩm từ phản hồi
					...
					$scope.totalViolationPages = res.data.totalPages;
					console.log($scope.listProducts);
				})
				.catch(function (error) {
					console.log(error);
		...
		$scope.getDescriptionList = function (productId) {
			$http
				.get(url + "/staff/violationsproduct/listDescription/" + productId)
				.then(function (res) {
		...
		$scope.getCountViolation = function (productId) {
			$http
				.get(url + "/staff/violationsproduct/countViolations/" + productId)
				.then(function (res) {
		...
		function loadViolationData(page) {
			$http
				.get(url + "/staff/alreadyviolationsproduct/", {
					params: { page: page, size: $scope.pageSize },
				})
				.then(function (response) {
					$scope.listViolationProducts = response.data;
					updateViolationPagination();
				})
				.catch(function (error) {
					console.error("Lỗi khi lấy dữ liệu bài viết vi phạm:", error);
				});
		...
		function calculateTotalViolationPages() {
			$scope.totalViolationPages = $scope.listViolationProducts.totalPages;
		}
		...
		$scope.prevPage = function () {
			if ($scope.currentPage > 0) {
				$scope.currentPage--;
				loadProductData($scope.currentPage);
			}
		};

		$scope.prevViolationPage = function () {
			if ($scope.currentViolationPage > 0) {
				$scope.currentViolationPage--;
				loadViolationData($scope.currentViolationPage);
			}
		};
		...
		$scope.nextViolationPage = function () {
			if ($scope.currentViolationPage < $scope.totalViolationPages - 1) {
				$scope.currentViolationPage++;
				loadViolationData($scope.currentViolationPage);
			}
		};
		...
		function updateViolationPagination() {
			calculateTotalViolationPages();
			// Đảm bảo rằng trang hiện tại không vượt quá tổng số trang
			$scope.currentPage = Math.min($scope.currentPage, $scope.totalPages - 1);
		}
		...
		$scope.getViolationPagesArray = function () {
			return Array.from({ length: $scope.totalViolationPages }, (_, i) => i);
		};
		...
		$scope.searchProduct = function () {
			...
			$http
				.get(url + "/staff/searchViolationProduct", { params: { name: name } })
				.then(function (response) {
		...
		$scope.searchProductViolation = function () {
			...
			$http
				.get(url + "/staff/searchAlreadyViolationProduct", {
					params: { name: name },
				})
				.then(function (response) {
					var products = response.data; // Lấy danh sách sản phẩm từ phản hồi
		...
		$scope.detailProduct = function (violationId) {
			...
			$http
				.get(url + "/staff/violationsproduct/detailProduct/" + violationId)
				.then(function (response) {
						...
						if (result.isConfirmed) {
							...
							$http({
								...
								.then(function (acceptResponse) {
									...
									Swal.fire({
										...
										if (result.isConfirmed) {
											...
											$scope.listProducts = $scope.listProducts.filter(
												function (product) {
								...
								.catch(function (acceptError) {
									console.error(
										"Lỗi khi chấp nhận yêu cầu các bài viết:",
										acceptError
									);
									Swal.fire({
										position: "top",
				...
				.then(function (response) {
					$http({
						...
						Swal.fire({
							...
							// Check if the user clicked the "OK" button
							if (result.isConfirmed) {
								$scope.listProducts = $scope.listProducts.filter(function (
									product
								) {
									return product.violationId !== violationId;
								});
